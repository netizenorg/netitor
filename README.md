...
The general workflow we use looks like this:
  ...
  - test your changes using the example/demo page [`index.html`](https://github.com/netizenorg/netitor/blob/master/index.html) (you can access `ne`, the instance of the Netitor in your browser's console, and test things like `ne.theme = 'my-new-theme'` for example)
...
Because of the nature of the Web (it's always changing) scrapers constantly have to be updated, the eduscraper is no exception. For this reason there's a script `npm run update-eduscraper` which will update this dependency and should be run anytime the eduscraper repo has been udpated.
